...
/**
 * recycle-view Component api use 
 *  supply wx.createRecycleContext Management function 
 */
var RecycleContext = __webpack_require__(3);

/**
 * @params options Parameter object Object ， The structure of the expansion is as follows 
      id: recycle-view of id
      dataKey: recycle-item of wx:for Binded data variable 
      page: recycle-view Examples of the page or component 
      itemSize:  Function or Object Object ， Generate each recycle-item Width and height 
 * @return RecycleContext Object 
 */
...
function RecycleContext(_ref) {
  ...
  this.page = page;
  // Add the root parameter for use for useInpage alone
  this.root = root;
  ...
  if (this.useInPage) {
    ...
    // Rewrite the onPageScroll event
...
RecycleContext.prototype._forceRerender = function (id, cb) {
  this.isDataReady = true; // The first call indicates that the data has been ready
  // Dynamic calculation height and cache
  var that = this;
  var allrect = null;
  var parentRect = null;
  var count = 0;
  ...
  function newcb() {
    ...
    // Calculate Placeholder, only support when the dynamic calculation height
...
RecycleContext.prototype._recalculateSizeByProp = function (list, cb) {
  ...
  function sizeReady(rects) {
    ...
    if (itemSize.cacheKey) {
      wx.setStorageSync(itemSize.cacheKey, propValueMap); // Cchestrate the data
    }
    if (cb) {
      cb(sizeData);
    }
...
// Call this interface when the two slots be changed and after change
RecycleContext.prototype._recalculateSize = function (list) {
  // Traversing all data
  // It should be at least a thousand levels, there is no problem traversal
  var sizeMap = {};
  ...
  // Demolition the entire page into many squares of 200*200 to determine which square of each data is on
  for (var i = 0; i < listLen; i++) {
    list[i].__index__ = i;
    var itemSize = {};
    // Get the width and height of each item
    if (funcExist) {
      // It must be ensured that the height of each line returned is the same
    ...
    // Determine which square of the data falls on
    // Beyond the width, move to the next line, and then judge whether it is necessary to move to the next square according to the height
    if (offsetLeft + itemSize.width > compData.width) {
      column = 0;
      offsetLeft = itemSize.width;
      // Fixed issue #22
      if (sizeArray.length >= 2) {
        offsetTop += sizeArray[sizeArray.length - 2].height || 0; // Add the height of the last data
      } else {
        offsetTop += itemSize.height;
      }
      // Offsettop += SIZEARAY [SIZEARRAY.Length- 2] .height // Add the height of the last data
      // Judging whether it is necessary to move to the next square according to height
      if (offsetTop >= RECT_SIZE * (line + 1)) {
        // fix: When the block is relatively large, there will be no area information about the block
        var lastIdx = i - 1;
        var lastLine = line;
      ...
      // The new line of elements, Beforeheight is the addition of BeForeheight and Height of the previous element
      if (i === 0) {
      ...
      if (i === 0) {
        ...
        // The elements of the same row, BeForeheight are the same as the BeForeheight of the previous element
...
// Re -update the data on the next page
...
module.exports = function (e, cb) {
  ...
  // Boundary value judgment to avoid abnormalities. Assuming that the CreaterecycleContext is called first, and then delay 2s calling the APPEND inserting data
  ...
  item.pos = pos;
  // Add IGNOREBEGINDEX andignoreEndIndex
  ...
  // Fix #1
  // Remove BATCHSETDATAKEY and support 2 display in one pagerecycle-view
  // const groupSetData = () => {
  //   this.setData(obj)
