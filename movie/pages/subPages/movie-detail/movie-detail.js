...
Page({
  ...
  initPage(movieId){
    ...
    wx.request({
      ...
      success(res) {
        wx.hideLoading();
        const { movie, celebrities } = res.data;
        ...
        _this.setData({
          detailMovie: _this.handleData(movie),
          celebrities
        })
      }
    })
  ...
  formatData(arr) {
    ...
    if (arr.length) {
      list = arr.map(item => {
        ...
        temp.stars = temp.score ? this.formatStar(temp.score) : []
  ...
  handleData(data){
    ...
    //评分星星,满分10分，一颗满星代表2分
    obj.stars = obj.sc ? this.formatStar(obj.sc/2) : [];
    //处理媒体库的图片
  ...
  toVideo(){
    ...
    wx.navigateTo({
      url: `/pages/subPages/video-page/video-page?paramsStr=${encodeURIComponent(paramsStr)}`
    })
  }
