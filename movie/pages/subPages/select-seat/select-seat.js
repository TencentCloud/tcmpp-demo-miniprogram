 ...
 Page({
   ...
   onLoad: function(params) {
     let that = this;
     const info = JSON.parse(params.paramsStr);
     this.setData({
        ticketInfo: info
     });

     wx.getSystemInfo({
        success: ({ screenHeight, screenWidth, statusBarHeight  }) => {
          this.setData({
            seatArea: screenHeight - statusBarHeight - (500 * screenWidth / 750),
            rpxToPx: screenWidth / 750
          });
        }
    });
   },
     ...
     if (result.errorCode == 0) {
       ...
       that.setData({
         ...
         seatList: seatList.map(item => {
            item.price = this.data.ticketInfo.price;
            return item;
         }),
   ...
   clickSeat: function(event) {
     ...
     let totalPrice = this.data.ticketInfo.price * _selectedSeatList.length;
   ...
   processUnSelected: function(index) {
     ...
     if (otherLoveSeatIndex !== null) {
       ...
       if (_selectedSeatList.length >= this.data.maxSelect - 1) {
         wx.showToast({
           title: i18n.t('最多只能选择{value}个座位哦~', {
            value: this.data.maxSelect
           }),
           icon: 'none',
           duration: 2000
         })
   ...
   createOrder: function() {
     ...
     if (selectSeatInfo) {
       for (var i = 0; i < selectSeatInfo.length; i++) {
         ...
         seatInfo.push(i18n.t(`{row}排{column}座`, {
          row: selectSeatInfo[i].row,
          column: selectSeatInfo[i].col
         }))
       }
     }
