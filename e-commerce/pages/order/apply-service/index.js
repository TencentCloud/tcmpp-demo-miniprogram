...
Page({
  ...
  data: {
    ...
    goodsInfo: {},
    receiptStatusList: [
      { desc: 'Not received', status: ServiceReceiptStatus.NOT_RECEIPTED },
      { desc: 'Received', status: ServiceReceiptStatus.RECEIPTED },
    ],
    applyReasons: [],
    serviceType: null, // 20-仅退款，10-退货退款
    serviceFrom: {
      returnNum: 1,
      receiptStatus: { desc: 'Select', status: null },
      applyReason: { desc: 'Select', type: null },
  ...
  validate() {
    ...
    if (!this.data.serviceFrom.applyReason.type) {
      valid = false;
      msg = i18n.t('Please enter reason for refund');
    } else if (!this.data.serviceFrom.amount.current) {
      valid = false;
      msg = i18n.t('Please enter refund amount');
    }
    if (this.data.serviceFrom.amount.current <= 0) {
      valid = false;
      msg = i18n.t('The amount must be greater than 0');
    }
  ...
  checkQuery() {
    ...
    if (!orderNo) {
      Dialog.alert({
        content: i18n.t('Please select order first'),
      }).then(() => {
    ...
    if (!skuId) {
      Dialog.alert({
        content: i18n.t('Please select product first'),
      }).then(() => {
  ...
  async refresh() {
    ...
    try {
      ...
      this.setData({
        ...
        amountTip: `You can get a maximum refund amount ¥ ${priceFormat(
          res.data.refundableAmount,
          2,
        )}, including shipping cost ¥ ${priceFormat(res.data.shippingFeeIncluded, 2)}`,
  ...
  onApplyOnlyRefund() {
    wx.setNavigationBarTitle({ title: 'Apply for Refund' });
    this.setData({ serviceRequireType: 'REFUND_MONEY' });
    this.switchReceiptStatus(0);
  },

  onApplyReturnGoods() {
    wx.setNavigationBarTitle({ title: 'Apply for Return and refund' });
      ...
      .then(() => {
        if (orderStatus === OrderStatus.PENDING_RECEIPT) {
          return Dialog.confirm({
            title: i18n.t('Have you received the purchased product?'),
            content: '',
            confirmBtn: i18n.t('Yes. Apply for return and refund'),
            cancelBtn: i18n.t('No'),
          }).then(() => {
  ...
  onApplyReturnGoodsStatus() {
    reasonSheet({
      show: true,
      title: i18n.t('Select reason for the refund'),
      options: this.data.applyReasons.map((r) => ({
        title: r.desc,
      })),
      showConfirmButton: true,
      showCancelButton: true,
      emptyTip: i18n.t('Please select the reason for the refund'),
    }).then((indexes) => {
      this.setData({
  ...
  onApplyGoodsStatus() {
    reasonSheet({
      show: true,
      title: i18n.t('Please select the product package status'),
      options: this.data.receiptStatusList.map((r) => ({
        title: r.desc,
      })),
      showConfirmButton: true,
      emptyTip: i18n.t('Select the product package status'),
    }).then((indexes) => {
      this.setData({
  ...
  switchReceiptStatus(index) {
    ...
    if (!statusItem) {
      this.setData({
        showReceiptStatusDialog: false,
        'serviceFrom.receiptStatus': { desc: i18n.t('Please select'), status: null },
        'serviceFrom.applyReason': { desc: i18n.t('Please select'), type: null }, // 收货状态改变时，初始化申请原因
        applyReasons: [],
      });
      return;
    }
    ...
    this.getApplyReasons(statusItem.status).then((reasons) => {
      this.setData({
        ...
        'serviceFrom.applyReason': { desc: i18n.t('Please select'), type: null }, // 收货状态改变时，重置申请原因
      ...
      dispatchApplyService(params)
        .then((res) => {
          Toast({
            context: this,
            selector: '#t-toast',
            message: i18n.t('application is successful'),
            icon: '',
          });
