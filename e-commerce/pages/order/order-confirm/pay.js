...
export const paySuccess = (payOrderInfo) => {
  ...
  Toast({
    context: this,
    selector: '#t-toast',
    message: i18n.t('Paid successfully.'),
    duration: 2000,
    icon: 'check-circle',
  });
...
export const payFail = (payOrderInfo, resultMsg) => {
  if (resultMsg === 'requestPayment:fail cancel') {
    if (payOrderInfo.dialogOnCancel) {
      //结算页，取消付款，dialog提示
      Dialog.confirm({
        title: i18n.t('Are you sure you want to discard the payment?'),
        content: i18n.t('The product will be sold out soon. Would you like to discard the payment?'),
        confirmBtn: i18n.t('Yes'),
        cancelBtn: i18n.t('Continue payment'),
      }).then(() => {
    ...
    Toast({
      context: this,
      selector: '#t-toast',
      message: i18n.t('Payment failed: {resultMsg}', { resultMsg }),
      duration: 2000,
      icon: 'close-circle',
    });
    setTimeout(() => {
